...
1. **Segmentation optimization and prompt refinement**  
   Original Group 1 work on diagnosing validation problems and improving the WO2Net segmentation/prompting workflow.
...
├── data/
│   ├── vtt_files/
│   │   └── Place raw .vtt interview files here.
│   │       These files are not included on GitHub.
│   │
│   └── crowdsource_data/
│       ├── enriched_segments.json
│       │   └── Aggregated from original individual enriched segment files.
│       │
│       └── segment_validations.json
│           └── Aggregated from original individual validation files.
│
└── results/
    ├── ready_prompts/
    ...
    │
    ├── json_outputs/
    │   └── Raw JSON outputs from ChatGPT.
    │
    └── analysis_reports/
        └── Generated reports comparing performance.
```
...
Baseline prompting uses:

- the segment excerpt
- the original candidate concepts from the previous WO2 oral history matching pipeline
- the original archived prompt logic

This condition can only select from the original candidate list.

### 2. Refined Prompting

Refined prompting uses:

- the same segment excerpt
- the same original candidate concepts
- stricter top-down prompt logic from the earlier prompt-refinement work

This condition tests whether stricter prompting improves concept selection when the candidate list stays the same.

### 3. KG-RAG Prompting

KG-RAG prompting uses:

- the segment excerpt
- retrieved candidate concepts from the WO2 Thesaurus
- LLM-based selection from the retrieved candidate list
...
The KG-RAG retrieval step uses four layers.

### 1. Original Seed Concepts

Concepts already matched by the previous WO2Net pipeline are kept as seed candidates. This prevents useful original concepts from being lost.

### 2. Explicit Lexical Retrieval

The segment text is matched against searchable WO2 Thesaurus labels. This layer retrieves concepts whose labels are directly mentioned in the segment, such as named places, camps, organisations, or events.

### 3. Contextual Event Retrieval

This layer searches specifically for event concepts supported by combinations of contextual clues, such as:
...
This allows the retriever to suggest event candidates even when the exact event label is not quoted literally.

### 4. Conservative Event Graph Expansion

For the strongest contextual event candidates, the retriever may add a small number of related or broader event concepts from the thesaurus graph.

This expansion is restricted to event concepts and intentionally limited to reduce noisy matches.
...
From this shortlist, five segments were selected for expert evaluation. These cases were selected because they represented different concept-linking problems:
...
- Experts rated each proposed concept for:
  - correctness
  - specificity
...
For WO2Net, a future KG-RAG-style pipeline would require:

- careful retrieval design
- strict LLM selection
- URI-based concept normalization
- expert validation
- separation between segment-level concept linking and broader interview-level thematic interpretation

---

## Thesis Repository Structure

The thesis work is located in:
...
│   │   └── Selects the five evaluation cases from enriched segments and validation groups.
│   │
│   ├── retrieve_candidates.py
│   │   └── Defines the KG-RAG candidate retrieval logic.
│   │
│   ├── 04_run_candidate_retrieval.py
│   │   └── Runs KG-RAG candidate retrieval for the selected evaluation segments.
│   │
│   ├── 05_build_kg_rag_prompts.py
│   │   └── Builds KG-RAG prompts from selected segments and retrieved thesaurus candidates.
│   │
│   ├── 08_build_refined_prompts.py
│   │   └── Builds refined prompts using original WO2Net candidate concepts and stricter prompt logic.
│   │
│   ├── 10_build_baseline_prompts.py
│   │   └── Builds baseline prompts using original WO2Net candidate concepts and archived baseline prompt logic.
│   │
│   └── 14_process_expert_evaluation.py
│       └── Processes expert questionnaire responses and generates summary tables.
│
└── results/
    ├── llm_prompts/
    │   ├── baseline/

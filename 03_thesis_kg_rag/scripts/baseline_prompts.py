...
This file contains the original baseline prompt logic from the previous WO2Net
optimization work.

Original source file:
    original_prompts_archive.py

In the bachelor thesis pipeline, this file is used as the baseline prompting
condition for comparison with:
    1. refined prompting
    2. KG-RAG prompting
...
Important:
    The original file name was `original_prompts_archive.py`, because it was
    kept as an archive of the older prompt logic in the previous project.

    In this thesis folder, the file is renamed to `baseline_prompts.py` because
    it is actively used as the baseline comparison condition.

    The prompt logic should remain unchanged except for minimal changes needed
    to make it importable and usable in the thesis pipeline.
"""

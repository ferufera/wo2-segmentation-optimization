...
This file contains prompt builder functions from the previous WO2Net
segmentation optimization work.

In the bachelor thesis pipeline, this file is used only for the refined
prompting comparison condition.

Relevant thesis function:
    _build_topdown_matching_prompt()

This function builds a prompt that asks the LLM to select the most relevant
concepts from an existing list of original WO2Net matched concepts.

Pipeline role:
    evaluation segment
        -> original WO2Net matched concepts
        -> refined prompt
        -> LLM concept selection
        -> refined prompting output

Important:
    This file was copied from the earlier 02_optimization work into the thesis
    folder to keep the thesis pipeline self-contained and reproducible.

    Not all functions in this file are used in the thesis experiment. Some
    functions are kept because they were part of the original refined prompt
    module and may be useful for context or future reference.
"""

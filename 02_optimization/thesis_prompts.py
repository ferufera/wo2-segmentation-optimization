...
def _build_topdown_matching_prompt(concept_labels: list[str], segment_text: str) -> str:
    """
    Thesis version:
    stronger guidance toward precise thesaurus concepts instead of broad categories.
    """
...
- Prefer precise thesaurus concepts over broad categories whenever possible.
- Focus especially on specific events, places, organizations, and historically meaningful entities.
- Use only the concepts exactly as listed. Do not invent or modify concept names.

Rules:
1. If a specific event, place, organization, or person is central to the fragment and appears in the list, select it instead of a broad concept.
2. Prefer narrower, deeper thesaurus concepts over broader ones when both are possible.
3. Only include broad categories if no more specific concept in the list captures the segment.
4. Select a concept only if it is central to the fragment as a whole, not based on a single word match.
5. If the fragment is too generic and does not clearly contain specific historical information, return fewer concepts rather than broad, weak matches.
6. Use concept labels that help describe who, what, when, and where when these are clearly present.
7. Do NOT include any concept with a score of 0.5 or lower.
8. Never return concepts that are not present in the list.
